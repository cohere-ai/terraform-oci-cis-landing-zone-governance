...
resource "oci_budget_budget" "these" {
  ...
  start_date                            = each.value.schedule != null ? (each.value.schedule.processing_period_type == "SINGLE_USE" ? (each.value.schedule.single_use_start_date != null ? "${each.value.schedule.single_use_start_date}T00:00:00.001-00:00" : "${trimsuffix(timestamp(), "Z")}.001-00:00") : null) : null
  end_date                              = each.value.schedule != null ? (each.value.schedule.processing_period_type == "SINGLE_USE" ? (each.value.schedule.single_use_end_date != null ? "${each.value.schedule.single_use_end_date}T23:59:59.001-00:00" : "${trimsuffix(timeadd(timestamp(), "720h"), "Z")}.001-00:00") : null) : null
